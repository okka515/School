...
int get_list_size(ListPtr l); //プロトタイプ宣言
void print_list_in_reverse(ListPtr l)
{
    ...
    int *list_stock = malloc(list_size * sizeof(int));
    int list_top = 0;

    NodePtr node = l->head;
    while (node != NULL) {
        list_stock[list_top] = node->val;
        node = node->next;
        list_top++;
    }
...
{
    /*挿入周辺ノードの宣言*/
    NodePtr addNode = NULL;
    NodePtr current_Node = l->head;
    int current_index = 0;

    /*追加ノードの作成*/
    addNode = create_node(val);

    /*先頭Nodeに挿入する場合*/
    if (index == 0) {
        add_first(l, val); //add_first関数の使用
    }
    ...
    else
    {
        ...
        /*Node挿入処理*/
        addNode->next = current_Node->next;
